...
func fuelConsumption(t int, positions []int) int {
	f := 0
...
func lowestConsumption(positions []int) (int, int) {
	...
	curConsumption := fuelConsumption(curPos, positions)
	for {
		lowerCons := fuelConsumption(curPos-1, positions)
		higherCons := fuelConsumption(curPos+1, positions)
		if lowerCons >= curConsumption {
			if higherCons >= curConsumption {
				break
			} else {
				curPos++
				curConsumption = higherCons
			}
		} else {
			curPos--
			curConsumption = lowerCons
		}
	}
	return curPos, curConsumption
}

func TestDay7Phase1(t *testing.T) {
	require.Equal(t, 37, fuelConsumption(2, light))
	require.Equal(t, 41, fuelConsumption(1, light))
	require.Equal(t, 39, fuelConsumption(3, light))
	require.Equal(t, 71, fuelConsumption(10, light))

	p, fc := lowestConsumption(light)
	require.Equal(t, 2, p)
	require.Equal(t, 37, fc)

	p, fc = lowestConsumption(large)
	assert.Equal(t, 361, p)
	assert.Equal(t, 364898, fc)
}

func mean(positions []int) float64 {
	v := 0
	for _, p := range positions {
		v += p
	}
	return float64(v) / float64(len(positions))
}

func quadFuelConsumption(t int, positions []int) int {
	fc := 0
	for _, p := range positions {
		var lfc int
		if p > t {
...
func TestDay7Phase2(t *testing.T) {
	require.Equal(t, 168, quadFuelConsumption(5, light))
	require.Equal(t, 206, quadFuelConsumption(2, light))

	m := mean(light)
	require.Equal(t, 4.9, m)
	require.Equal(t, 170, quadFuelConsumption(int(math.Floor(m)), light))
	require.Equal(t, 168, quadFuelConsumption(int(math.Ceil(m)), light))

	m = mean(large)
	assert.Equal(t, 500.55, m)
	require.Equal(t, 104149091, quadFuelConsumption(int(math.Floor(m)), large))
	require.Equal(t, 104149135, quadFuelConsumption(int(math.Ceil(m)), large))
}
